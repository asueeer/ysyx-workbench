...
static word_t sw(word_t i) { // sext(i[31:0])
    return SEXT(BITS(i, 31, 0), 32);
}

static word_t w(word_t i) { // i[31:0]
    return BITS(i, 31, 0);
}
...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  __VA_ARGS__ ; \
    ...
    INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge, B, s->dnpc = src1 > src2 ? s->pc + dest : s->dnpc);
    INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt, B, s->dnpc = src1 < src2 ? s->pc + dest : s->dnpc);
    ...
    INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw, R, R(dest) = sw(w(src1 * src2)));
    INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw, R,
            R(dest) = sw(sw(src1) / sw(src2)));
    INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw, R, R(dest) = sw(sw(src1) % sw(src2)));
